...
if (data.jobs.some(job => job.apply_url && forbiddenHosts.test(new URL(job.apply_url).hostname))) throw new Error("An aggregation link was exposed as an application URL.");
if (!data.jobs.some(job => job.apply_url)) throw new Error("No verified application URLs were published.");
const html = readFileSync(resolve(root, "public/index.html"), "utf8");
if (!html.includes("法律职位检索") || html.includes("法律职位雷达")) throw new Error("Website title was not updated.");
if (!html.includes("a.href=escUrl(job.apply_url)") || html.includes("a.href=escUrl(job.url)")) throw new Error("Website links are not using verified application URLs.");
